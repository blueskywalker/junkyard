...
app.controller("data",function($scope,$http) {
    ...
    $scope.change = function() {
        ago = (Date.now()/1000) - $scope.period;

        sql="select topic, sum(logsize) as logsize,sum(offset) as offset,sum(lag) as lag,timestamp from `"+ makeDBName($scope.cluster);
        sql +="`.`"+$scope.group+"` where timestamp > " + ago + " and LTRIM(topic) = '" +$scope.topic+"' group by timestamp;";

        var url="get_datasource.php?sql="+sql;
